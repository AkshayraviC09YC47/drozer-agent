...
import com.reversec.dz.views.ConnectorStatusIndicator;
import com.reversec.dz.views.logger.LogMessageAdapter;
...
public class ServerActivity extends ConnectorActivity implements Observer, Server.OnDetailedStatusListener {
    ...
    protected void startServer() {
		try {
			...
			this.parameters.setStatus(com.reversec.jsolar.api.connectors.Server.Status.UPDATING);
			
			Agent.getInstance().getServerService().startServer(Agent.getInstance().getServerParameters(), Agent.getInstance().getMessenger());
		}
		catch(RemoteException e) {			
			this.parameters.setStatus(com.reversec.jsolar.api.connectors.Server.Status.OFFLINE);
